...
Classification priority: risk_active hard-blocks to "blocked" regardless
of score. Otherwise, >= FREEZE_MIN_VERIFIED_PLACEMENTS verified placements
that produced zero tracked clicks is evidence of "tested and dead" and
freezes the title even at score 0 — but a zero-signal title with fewer
placements than that is merely untested ("test"), not frozen. Above the
freeze check, score alone picks "scale" / "maintain" / "test".
...
PLACEMENTS_CAP = 5.0
# conversion_signal is expected as a 0..1 fraction and normalized directly.
...
def _component_scores(title: dict) -> dict:
    ...
    return {
        ...
        "conversion_signal": _normalize(conversion_signal, 1.0),
...
def _reasons(
    ...
    if classification == "blocked":
        reasons.append("risk_active flag hard-blocks regardless of score")
